...
bool RFM69::initialize(byte freqBand, byte nodeID, byte networkID)
{
  const byte CONFIG[][2] =
  {
    ...
    /* 0x02 */ { REG_DATAMODUL, RF_DATAMODUL_DATAMODE_PACKET | RF_DATAMODUL_DATAMODE_CONTINUOUS | RF_DATAMODUL_MODULATIONSHAPING_00 }, //no shaping
...
void RFM69::writeRegs(byte addr1, byte value1, byte addr2, byte value2)
{
  select();
  SPI.transfer(addr1 | 0x80);
  SPI.transfer(value1);
  SPI.transfer(addr2 | 0x80);
  SPI.transfer(value2);
  unselect();
}
